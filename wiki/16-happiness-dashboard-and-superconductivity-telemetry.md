...
* Человек в ловушке валидации радуется чужому одобрению, не замечая, что его генератор Тандэн заблокирован паразитным обратным током.
...
flowchart LR
    subgraph Inputs ["Входные физические параметры цепи"]
        ...
        I["Вектор тока I (Прямой vs Обратный)"]
    ...
    subgraph Formula ["Ядро вычисления КПД"]
        ...
        EtaI["η_polarity (1.0 при I>0, 0.2 при I<0)"]
    ...
    subgraph HUD ["Дашборд счастья в реальном времени"]
        ...
        Mode["Режим: Мусин / Номинал / Перегрев / Обратный ток"]
...
* **Прямой ток ($I > 0$):** Тандэн генерирует ток и отдает его во внешнее действие (код, чашка чая, забота, музыка). $\eta_{\text{polarity}} = 1.0$.
* **Паразитный обратный ток ($I < 0$):** Попытка «зарядиться» от лампочки (требование похвалы, страх критики, зависимость от лайков). Внешняя нагрузка не имеет генератора — линия закольцовывается, проводка вскипает. $\eta_{\text{polarity}} = 0.18 - 0.20$.
...
| **2. Статус-бейдж** | **Режим контура** | `MUSHIN`, `NOMINAL`, `OVERHEAT`, `REVERSE_TRAP` | Мгновенный код состояния оператора. |
| **3. Сенсор $R$** | **Импеданс внимания** | `0.1 – 100+ Ω` | Суммарный паразитный шум ума (симуляции прошлого, будущего и внутреннего цензора). |
| **4. Сенсор $I$** | **Вектор тока** | `I > 0` / `I < 0` | Направление потока: созидание наружу vs высасывание валидации внутрь. |
...
| :--- | :--- | :--- |
| **Основа метрики** | Субъективное самочувствие («мне грустно») | Термодинамическое сопротивление проводника $R$ и вектор $I$ |
| **Причинность** | «День не задался», «ретроградный меркурий» | Омический перегрев Джоуля — Ленца, паразитный обратный ток |
...
| **Протокол ремонта** | «Подышите», «выпейте кофе», «улыбнитесь» | Сброс $t_{\text{past}} \to 0, t_{\text{future}} \to 0$, разворот вектора полярности через Нидзиригути, защита Дзансин |
