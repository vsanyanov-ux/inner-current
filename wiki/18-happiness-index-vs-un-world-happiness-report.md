...
| **Отношение к парадоксам** | Богатый человек в депрессии ломает статистику модели | Модель сразу видит омический перегрев ($Q \gg 0$) или короткое замыкание на Эго |
...
flowchart TD
    ...
    subgraph Diode ["Вектор внимания"]
        Direct["I > 0: Прямая отдача в мир (Созидание)"]
        Reverse["Петля КЗ: Замыкание на Эго (Ловушка валидации)"]
    end
    ...
    Reverse -.->|Петля КЗ на эго| ScoreTrap["Индекс Счастья H: 18%"]
```
...
2. **$\eta_{\text{полярность}}$ (Вектор внимания и чистота отдачи):**
   Показывает, куда направлена мощность ядра. Если человек созидает ради самого процесса ($I > 0$), ток течет во внешний мир. Если он делает это ради похвалы, лайков или страха осуждения, внимание срывается в паразитное короткое замыкание на собственном «Я», выжигая самооценку.
